...
const DeactivatedUser = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5000/api/v1/users/deactivated', config);
  ...
  const handleReactivate = async () => {
    try {
      console.log('Selected Users for Reactivation:', selectedUsers); // Debugging: Log the selected users
      const token = localStorage.getItem('token'); // Get the token from localStorage
      const config = {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      await axios.put(
        'http://localhost:5000/api/v1/users/reactivate',
        { userIds: selectedUsers },
        config
      );

      toast.success('Selected users have been reactivated.');
      fetchUsers(); // Refresh the user list
    } catch (error) {
      console.error('Error reactivating users:', error);
      toast.error('Failed to reactivate users.');
    }
  };
  ...
  useEffect(() => {
    console.log('Selected Users State:', selectedUsers); // Debugging: Log the selected users state
  }, [selectedUsers]);
      ...
      <div className="container mx-auto p-4">
        ...
        <Box mb={2} display="flex" justifyContent="center" gap={2}>
          <Button
            ...
            color="success"
            onClick={() => {
              console.log('Reactivate button clicked'); // Debugging: Log button click
              handleReactivate();
            }}
